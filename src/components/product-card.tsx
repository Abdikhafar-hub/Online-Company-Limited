...
export function ProductCard({ product }: { product: Product }) {
  const wa = buildWhatsAppLink(`Hello Online Company Limited, I'd like to order: ${product.name}. Please share availability and final price.`);
  return (
    <article className="group bg-white rounded-3xl border border-beige-border overflow-hidden flex flex-col transition-all hover:-translate-y-1 hover:shadow-[0_24px_50px_-30px_rgba(11,19,43,0.35)]">
      <div className="p-4 pb-2">
        <div className="relative">
          <span className="absolute z-10 top-2 left-2 text-[0.7rem] tracking-[0.16em] uppercase font-semibold bg-white/85 text-navy-soft px-2.5 py-1 rounded-full">{product.brand}</span>
      ...
      <div className="px-5 pb-5 pt-2 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-baseline gap-2">
          {product.requestPrice ? (
            ...
            <>
              ...
              {product.oldPrice && <span className="text-sm text-muted-foreground line-through">{formatKES(product.oldPrice)}</span>}
            </>
          )}
        </div>
        ...
        <div className="mt-auto pt-4 flex items-center justify-between border-t border-beige-border">
          <Link to="/products/$slug" params={{ slug: product.slug }} className="text-sm font-semibold text-navy underline-offset-4 hover:underline">
            View details
          </Link>
          <a href={wa} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-whatsapp hover:bg-whatsapp-dark text-white text-sm font-semibold transition-colors">
